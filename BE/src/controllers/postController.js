...
const postController = {
    ...
    async getEventPosts(req, res) {
        try {
            ...
            if (event.approval_status !== 'approved') {
                return res.status(403).json({ success: false, message: "Kênh trao đổi chưa mở." });
            }
            ...
            if (!canView) {
                return res.status(403).json({
                    success: false,
                    message: "Bạn không có quyền xem nội dung này. Chỉ thành viên tham gia sự kiện mới được truy cập."
                });
            }

            // Lấy dữ liệu (có phân trang)
            const result = await Post.getByEventId(event_id, page, limit);
    ...
    async createPost(req, res) {
        try {
            ...
            if (event.approval_status !== 'approved') {
                return res.status(403).json({ success: false, message: "Kênh trao đổi chưa được mở." });
            }
            ...
            if (!canPost) {
                return res.status(403).json({ success: false, message: "Bạn không có quyền đăng bài." });
            }
    ...
    async deletePost(req, res) {
        try {
            ...
            if (post.event_is_deleted || post.event_status === 'rejected') {
                return res.status(403).json({
                    success: false,
                    message: "Hành động bị từ chối: Sự kiện này đã bị xóa hoặc bị hủy bỏ (Đóng băng dữ liệu)."
                });
            }
            ...
            if (!canDelete) {
                return res.status(403).json({
                    success: false,
                    message: "Bạn không có quyền xóa bài viết này (Chỉ tác giả hoặc Quản lý sự kiện mới được xóa)."
                });
            }
